  ...
  useEffect(() => {
    ...
    if (canvas) {
      ...
      sketch = new InfiniteGridMenu(
        ...
        canvas,
        items.length ? items : [{ image: 'https://picsum.photos/500/500?random' }],
        handleActiveItem,
        setIsMoving,
        sk => sk.run(),
        scale
...
const CardSwap = ({ width = 400, height = 320, cardDistance = 60, verticalDistance = 70, delay = 8000, skewAmount = 6, children, onCardClick }) => {
 
  const config = { ease: 'elastic.out(0.6,0.9)', durDrop: 1.2, durMove: 1.2, durReturn: 1.2, promoteOverlap: 0.9, returnDelay: 0.05 };
...
//
  ...
  useEffect(() => {
    ...
    const swap = () => {
      ...
      if (!elFront) { isAnimatingRef.current = false; return; }
  ...
  const rendered = childArr.map((child, i) => {
    ...
    return isValidElement(child) ? cloneElement(child, {
      ...
      key: i, ref: refs[i],
      style: { width, height, pointerEvents: isFront && !isAnimatingRef.current ? 'auto' : 'none', cursor: isFront && !isAnimatingRef.current ? 'pointer' : 'default', ...(child.props.style ?? {}) },
      onClick: e => {
    ...
    <div className="app-scroll app-bg-active">
      ...
      <section className="page page-2">
        ...
        <div className="page-2-left">
          ...
          <p>Spin the globe to look back at the moments that made this year special.</p>
        </div>
